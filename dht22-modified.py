...
def log_temperature(location,temp,humidity):

    conn=sqlite3.connect(dbname)
    curs=conn.cursor()

    curs.execute("INSERT INTO temps values(date('now'), time('now'), (?), (?), (?))", (location,temp,humidity))
...
def main():
        ...
        NotifyHost()
	log_temperature(location,temp,humidity)
